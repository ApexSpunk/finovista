...
function editService({ service }) {
  ...
  const fetchService = async () => {
    ...
    if (router.isReady) {
      ...
      if (!id) return null;
      try {
        const res = await fetch(`/api/singleService?slug=${id}`);
        const data = await res.json();
        setServiceid(data.services[0]._id);
        setServiceTitle(data.services[0].title);
        setThumbnail(data.services[0].thumbnail);
        setSlug(data.services[0].slug);
  ...
  async function updateService() {
    ...
    try {
      ...
      toast.success(`Service ${serviceTitle} Updated`, {
        position: "top-center",
        autoClose: 5000,
  ...
  const deleteService = async () => {
    try {
      ...
      let responseData = await response.json();
      toast.error(`Service ${serviceTitle} Deleted`, {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  function handleTitleInput(serviceTitle) {
    setServiceTitle(serviceTitle);
    serviceTitle = serviceTitle.split(" ");
    serviceTitle = serviceTitle.join("-").toLowerCase();
    setSlug(serviceTitle);
  }
            ...
            <div className="col-span-9">
              <div>
                <input
                  ...
                  value={serviceTitle}
                  onChange={(eve) => {
                    setServiceTitle(eve.target.value);
                  }}
            ...
            <div className="col-span-3">
              <div className="bg-white p-4 rounded-xl">
                ...
                <div className="grid mt-4  gap-2">
                  <button className="previewBtn w-full" onClick={updateService}>
                    Update Service
                  </button>
