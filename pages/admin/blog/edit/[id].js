...
function editPost({ posts }) {
  ...
  const fetchPost = async () => {
    ...
    if (router.isReady) {
      ...
      if (!id) return null;
      try {
        const res = await fetch(`/api/singlePost?slug=${id}`);
  ...
  async function updatePost() {
    ...
    try {
      ...
      toast.success(`Post ${postTitle} Updated`, {
        position: "top-center",
        autoClose: 5000,
  ...
  const deletePost = async () => {
    try {
      ...
      let responseData = await response.json();
      toast.error(`Post ${postTitle} Deleted`, {
        position: "top-center",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
            ...
            <div className="col-span-3">
              <div className="bg-white p-4 rounded-xl">
                ...
                <div className="grid mt-4  gap-2">
                  <button className="previewBtn w-full" onClick={updatePost}>
                    Update Post
                  </button>
